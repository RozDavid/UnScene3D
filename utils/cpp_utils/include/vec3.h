//
// Created by David Rozenberszki on 05.12.22., but taken from mLib by Niessner&Dai
//
...
public:
    ...
    //! Cross product
    inline vec3<T> operator^(const vec3 &other) const {
        return vec3<T>(array[1] * other.array[2] - array[2] * other.array[1],
                       array[2] * other.array[0] - array[0] * other.array[2],
                       array[0] * other.array[1] - array[1] * other.array[0]);
    }
    ...
    static T distSq(const vec3 &v0, const vec3 &v1) {
        return ((v0.array[0] - v1.array[0]) * (v0.array[0] - v1.array[0]) +
    ...
    static T dist(const vec3 &v0, const vec3 &v1) {
        return sqrt((v0.array[0] - v1.array[0]) * (v0.array[0] - v1.array[0]) +
                    (v0.array[1] - v1.array[1]) * (v0.array[1] - v1.array[1]) +
                    (v0.array[2] - v1.array[2]) * (v0.array[2] - v1.array[2]));
    }
    ...
    static vec3<T> randomUniform(T min, T max) {
        ...
        return vec3<T>(distribution(generator),
    ...
    inline void normalizeIfNonzero() {
        const T l = length();
        if (l == static_cast<T>(0)) { return; }  // Better to check against epsilon tolerance
    ...
    static T angleBetween(const vec3<T> &v0, const vec3<T> &v1) {
        ...
        if (l0 <= 0.0f || l1 <= 0.0f)
            ...
            return acosf(std::clamp(vec3<T>::dot(v0, v1) / l0 / l1, -1.0f, 1.0f)) * 180.0f / M_PI;
    }
...
namespace math {
    ...
    inline vec3<T> abs(const vec3<T> &p) {
        return vec3<T>(std::abs(p.x), std::abs(p.y), std::abs(p.z));
    }

    template<class T>
    inline vec3<T> square(const vec3<T> &v) {
        return vec3<T>(square(v.x), square(v.y), square(v.z));
    }
