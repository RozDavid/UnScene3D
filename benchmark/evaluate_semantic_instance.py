...
def compute_metric_averages(avg_values, metric='ap'):
    ...
    for (li, label_name) in enumerate(CLASS_LABELS):
        ...
        avg_dict["classes"][label_name][f"{metric}"] = np.average(avg_values[d_inf, li, oAllBut25])
        avg_dict["classes"][label_name][f"{metric}50%"] = np.average(avg_values[d_inf, li, o50])
        avg_dict["classes"][label_name][f"{metric}25%"] = np.average(avg_values[d_inf, li, o25])
    return avg_dict
...
def write_result_file(avg_aps, avg_ars, filename):
    ...
    with open(filename, 'w') as f:
        f.write(_SPLITTER.join(['class', 'class id', 'ap', 'ap50', 'ap25', 'ar', 'ar50', 'ar25']) + '\n')
        for i in range(len(VALID_CLASS_IDS)):
            ...
            f.write(_SPLITTER.join([str(x) for x in [class_name, class_id, ap, ap50, ap25, ar, ar50, ar25]]) + '\n')


def evaluate(preds: dict, gt_path: str, output_file: str, dataset: str = "scannet", gt_dict: dict = None, print_scene_results: bool = False):
    global CLASS_LABELS
    ...
    if dataset == "scannet200":
        CLASS_LABELS = (
            ...
            'refrigerator', 'lamp', 'kitchen cabinet', 'towel', 'clothes', 'tv', 'nightstand', 'counter', 'dresser', 'stool', 'cushion', 'plant', 'ceiling', 'bathtub', 'end table', 'dining table', 'keyboard', 'bag', 'backpack',
            'toilet paper',
            'printer', 'tv stand', 'whiteboard', 'blanket', 'shower curtain', 'trash can', 'closet', 'stairs', 'microwave', 'stove', 'shoe', 'computer tower', 'bottle', 'bin', 'ottoman', 'bench', 'board', 'washing machine', 'mirror',
            'copier',
            'basket', 'sofa chair', 'file cabinet', 'fan', 'laptop', 'shower', 'paper', 'person', 'paper towel dispenser', 'oven', 'blinds', 'rack', 'plate', 'blackboard', 'piano', 'suitcase', 'rail', 'radiator', 'recycling bin',
            'container',
            'wardrobe', 'soap dispenser', 'telephone', 'bucket', 'clock', 'stand', 'light', 'laundry basket', 'pipe', 'clothes dryer', 'guitar', 'toilet paper holder', 'seat', 'speaker', 'column', 'bicycle', 'ladder', 'bathroom stall',
            'shower wall',
            ...
            'furniture', 'cart', 'storage container', 'scale', 'tissue box', 'light switch', 'crate', 'power outlet', 'decoration', 'sign', 'projector', 'closet door', 'vacuum cleaner', 'candle', 'plunger', 'stuffed animal', 'headphones',
            'dish rack',
            'broom', 'guitar case', 'range hood', 'dustpan', 'hair dryer', 'water bottle', 'handicap bar', 'purse', 'vent', 'shower floor', 'water pitcher', 'mailbox', 'bowl', 'paper bag', 'alarm clock', 'music stand', 'projector screen',
            'divider',
            'laundry detergent', 'bathroom counter', 'object', 'bathroom vanity', 'closet wall', 'laundry hamper', 'bathroom stall door', 'ceiling light', 'trash bin', 'dumbbell', 'stair rail', 'tube', 'bathroom cabinet', 'cd case',
            'closet rod',
        ...
        VALID_CLASS_IDS = np.array((2, 4, 5, 6, 7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 21, 22, 23, 24, 26, 27, 28, 29, 31, 32, 33, 34, 35, 36, 38, 39, 40, 41, 42, 44, 45, 46, 47, 48, 49, 50, 51, 52, 54, 55, 56, 57, 58, 59, 62, 63, 64,
                                    65, 66, 67, 68, 69, 70, 71,
                                    72, 73, 74, 75, 76, 77, 78, 79, 80, 82, 84, 86, 87, 88, 89, 90, 93, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 110, 112, 115, 116, 118, 120, 121, 122, 125, 128, 130, 131, 132, 134, 136,
                                    138, 139, 140, 141, 145, 148, 154,
                                    155, 156, 157, 159, 161, 163, 165, 166, 168, 169, 170, 177, 180, 185, 188, 191, 193, 195, 202, 208, 213, 214, 221, 229, 230, 232, 233, 242, 250, 261, 264, 276, 283, 286, 300, 304, 312, 323, 325, 331, 342,
                                    356, 370, 392, 395, 399, 408, 417,
                                    488, 540, 562, 570, 572, 581, 609, 748, 776, 1156, 1163, 1164, 1165, 1166, 1167, 1168, 1169, 1170, 1171, 1172, 1173, 1174, 1175, 1176, 1178, 1179, 1180, 1181, 1182, 1183, 1184, 1185, 1186, 1187, 1188,
                                    1189, 1190, 1191))
    ...
    for i, (k, v) in enumerate(preds.items()):
        ...
        gt2pred, pred2gt = assign_instances_for_scan(v, gt_file) if gt_dict is None else assign_instances_for_scan_with_gt(v, gt_dict[k])
        matches[matches_key] = {}
...
def save_scene_results(matches, output_file):
    ...
    with open(scene_metrics_fname, 'w') as f:
        f.write(_SPLITTER.join(['class', 'scene_name', 'class id', 'ap', 'ap50', 'ap25', 'ar', 'ar50', 'ar25']) + '\n')
    ...
    for scene_path, scene_mathces in matches.items():
        ...
        with open(scene_metrics_fname, 'a') as f:
            for i in range(len(VALID_CLASS_IDS)):
                ...
                ar50 = avg_ars["classes"][class_name]["ar50%"]
                ar25 = avg_ars["classes"][class_name]["ar25%"]
                f.write(_SPLITTER.join([str(x) for x in [class_name, s_name, class_id, ap, ap50, ap25, ar, ar50, ar25]]) + '\n')
