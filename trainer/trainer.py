...
from utils.point_cloud_utils import separate_segments
...
class InstanceSegmentation(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        logs['train_mean_loss_noise_robust'] = statistics.mean(
            [item for item in [v for k, v in logs.items() if "loss_noise_robust" in k]])
    ...
    def save_visualizations(self, target_full, full_res_coords,
        ...
        if 'labels' in target_full:
            scannet_colors = np.stack(list(SCANNET_COLOR_MAP_200.values()))[1:]
            instances_colors = torch.from_numpy( scannet_colors[:target_full['labels'].shape[0]] )
        ...
        # Also add the top K most confident predictions for self training / proper visualization
        num_predictions = sorted_masks[did].shape[1]
        if topk is not None and num_predictions > 0:
            for inst_id in range(min(topk, num_predictions)):
                coords = np.copy(full_res_coords)
                inst_colors = np.zeros(coords.shape).astype(gt_pcd_color.dtype)
                inst_mask = sorted_masks[did][:, inst_id].astype(bool)
                inst_colors[inst_mask] = instances_colors[inst_id]
                v.add_points(f"TopK Instances;{inst_id}", coords[inst_mask],
                             colors=inst_colors[inst_mask],
                             normals=original_normals[inst_mask],
                             visible=False,
                             alpha=0.8,
                             point_size=point_size)

                inst_heatmap = sorted_heatmaps[did][:, inst_id]
                inst_heatmap_colors = cm.hot(inst_heatmap)[:, :3] * 255.
                v.add_points(f"TopK Heatmaps;{inst_id}", coords,
                             colors=inst_heatmap_colors,
                             normals=original_normals,
                             visible=False,
                             alpha=0.8,
                             point_size=point_size)
        ...
        # Save the predictions as np array for postporcessing and visualizing
        full_res_inst_ids = np.zeros((full_res_coords.shape[0], 4))
        full_res_inst_ids[:, :3] = full_res_coords
        for inst_id in range(num_predictions):
            inst_mask = sorted_masks[did][:, inst_id].astype(bool)
            full_res_inst_ids[inst_mask, -1] = inst_id
        np.save(f"{self.config['general']['save_dir']}/visualizations/{file_name}_instances.npy", full_res_inst_ids)
    ...
    def eval_step(self, batch, batch_idx):
        ...
        segment_connectivity = data.segment_connectivity
    ...
    def eval_instance_step(self, output, target_low_res, target_full_res, inverse_maps, file_names,
        ...
        for bid in range(len(prediction[self.decoder_id]['pred_masks'])):
            ...
            if self.config.general.filter_out_instances:
                ...
                normalization = pairwise_overlap.max(axis=0) + 10e-8
                ...
                for instance_id in range(norm_overlaps.shape[0]):
                    ...
                    # iterate over all the instances, with decreasing order in masks scores
                    # Filter out the instances under a given IoU score
                    if not(sort_scores_values[instance_id] < self.config.general.scores_threshold):
                        ...
                        if not sorted_masks[:, instance_id].sum() == 0.0:
                            ...
                            if len(overlap_ids) == 0:
                                ...
                                # only add the highest confidence score of the overlapping ones
                                # In most case this is probably already in there, or itself
                                if instance_id == min(overlap_ids):
                                    keep_instances.add(instance_id)

                # Instance separation based on connectivity
                keep_instances = sorted(list(keep_instances))
                if self.config.general.separate_instances and len(segment_connectivity[bid]) > 0:

                    # Get segment numbers and unique ids
                    unique_segment_ids = np.unique(target_full_res[bid]['point2segment'])
                    segment_nums = len(unique_segment_ids)

                    # Precompute connectivity dict
                    connectivity_dict = {}
                    for s_id in unique_segment_ids:
                        connectivity_dict[s_id] = set(segment_connectivity[bid][segment_connectivity[bid][:, 0] == s_id, 1])
                        connectivity_dict[s_id] = set.intersection(connectivity_dict[s_id], set(segment_connectivity[bid][segment_connectivity[bid][:, 1] == s_id, 0]))

                    batch_pred_classes, batch_pred_scores, batch_heatmaps, batch_masks = [], [], [], []
                    for inst_id in keep_instances:
                        # Map voxel to segments
                        segment_inst_mask = np.zeros(segment_nums, dtype=np.bool)
                        inst_mask = sorted_masks[:, inst_id].astype(np.bool)
                        segment_inst_mask_ids = np.unique(target_full_res[bid]['point2segment'][inst_mask])
                        segment_inst_mask[segment_inst_mask_ids] = True

                        # Separate connected foreground components based on connectivity
                        separated_instances = separate_segments(segment_inst_mask, unique_segment_ids, connectivity_dict)

                        # Add new instances to the list
                        batch_pred_classes += [sort_classes[inst_id]] * len(separated_instances)
                        batch_pred_scores += [sort_scores_values[inst_id]] * len(separated_instances)
                        batch_heatmaps += [sorted_heatmap[:, inst_id]] * len(separated_instances)
                        for sep_inst_id in range(len(separated_instances)):
                            batch_masks += [np.isin(target_full_res[bid]['point2segment'], separated_instances[sep_inst_id])]

                    all_pred_classes.append(torch.stack(batch_pred_classes))
                    all_pred_scores.append(np.array(batch_pred_scores))
                    all_heatmaps.append(np.stack(batch_heatmaps, axis=1))
                    all_pred_masks.append(np.stack(batch_masks, axis=1))
        ...
        for bid in range(len(prediction[self.decoder_id]['pred_masks'])):
            ...
            if self.config.general.save_for_freemask:

                # Create directory if it does not exist
                if not os.path.exists(f"{self.config['general']['save_dir']}/freemasks"):
                    os.makedirs(f"{self.config['general']['save_dir']}/freemasks")

                save_coords = full_res_coords[bid]
                save_colors = original_colors[bid]
                save_labels = np.zeros((save_coords.shape[0], 1))
                save_instances = np.zeros((save_coords.shape[0], 1))
                save_segment_ids = target_full_res[bid]['point2segment'][:, None].cpu().numpy()

                save_cloud = save_coords  # np.concatenate((save_coords, save_colors, save_labels, save_instances, save_segment_ids), axis=1)
                all_masks_save = all_pred_masks[bid].astype(bool)

                print(f'Saving {all_masks_save.shape[1]} masks for {file_names[bid]}')
                np.save(f"{self.config['general']['save_dir']}/freemasks/{file_names[bid]}_cloud.npy", save_cloud)
                np.save(f"{self.config['general']['save_dir']}/freemasks/{file_names[bid]}_masks.npy", all_masks_save)
    ...
    def eval_instance_epoch_end(self):
        ...
        if len(box_ap_50[0]) == 0 or len(box_ap_25) == 0:
            return
        ...
        if self.validation_dataset.dataset_name in ["scannet", "stpls3d", "scannet200", "freemask"]:
            ...
            with open(pred_path, "r") as fin:
                for line_id, line in enumerate(fin):
                    if line_id == 0:
                        # ignore header
                        continue
                    class_name, _, ap, ap_50, ap_25, ar, ar_50, ar_25 = line.strip().split(",")
    ...
    def test_epoch_end(self, outputs):
        ...
        dd['val_mean_loss_noise_robust'] = statistics.mean([item for item in [v for k, v in dd.items() if "loss_noise_robust" in k]])
