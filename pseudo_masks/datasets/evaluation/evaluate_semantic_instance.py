...
class Evaluator:
    ...
    def evaluate_matches(self, matches):
        ...
        ap = np.zeros( (len(self.distance_threshes) , len(self.CLASS_LABELS) , len(self.overlaps)))
        ar = np.zeros((len(self.distance_threshes), len(self.CLASS_LABELS), len(self.overlaps)))
    ...
    def compute_averages(self, avg_values, metric='ap'):
        d_inf = 0
    ...
    def print_results(self, avgs, metric='ap'):
        sep = ""
        col1 = ":"
        lineLen = 64
    ...
    def evaluate(self):
        ...
        print('')
        ap_scores, ar_scores = self.evaluate_matches(matches)
        avg_aps = self.compute_averages(ap_scores, metric='ap')
        avg_ars = self.compute_averages(ar_scores, metric='ar')


        # print
        self.print_results(avg_aps, metric='ap')
        self.print_results(avg_ars, metric='ar')
        return avg_aps['all_ap'], avg_aps['all_ap_50%'], avg_aps['all_ap_25%'], avg_ars['all_ar'], avg_ars['all_ar_50%'], avg_ars['all_ar_25%']
