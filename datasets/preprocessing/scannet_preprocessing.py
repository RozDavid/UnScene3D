...
class ScannetPreprocessing(BasePreprocessing):
    def __init__(
        ...
        if self.scannet200:
            self.labels_pd = pd.read_csv(self.data_dir / "scannet-labels.combined.tsv", sep='\t', header=0)  # Update to v2 if not from Canis
    ...
    def create_label_database(self, git_repo):
        if self.scannet200:
            ...
            df = pd.read_csv(self.data_dir / "scannet-labels.combined.tsv", sep="\t")
            df = (
