...
def load_model(args):
    ...
    return evaluator, transform, model

def main(args):

    lseg_model, lseg_transform, lseg_model = load_model(args)
    net_2d = lseg_model.net

    image = Image.open('data/example_input/0.jpg')
    input_labels = "chair, desk, wall, remote, cupboard, floor, curtain"
    pimage = lseg_transform(np.array(image)).unsqueeze(0).cuda()
    ...
    with torch.no_grad():
        b_size = 4
        image_features = net_2d.forward_image(pimage.expand(b_size, *pimage.shape[1:]))
