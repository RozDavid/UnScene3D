...
class LSeg(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        # Precompute text features during initialization
        self.precomputed_text = precompute_text_feats
        if self.precomputed_text:
            self.text = self.text.to(next(self.clip_pretrained.parameters()).device)
            self.precomputed_feats = self.clip_pretrained.encode_text(self.text).detach()
            self.clip_pretrained = None  # delete the model as we wont need it

            # Create a lookup dict that let's us map labels to index
            self.label_lookup = {}
            for index, label in enumerate(self.labels):
                self.label_lookup[label] = index
            self.label_index_mapper = np.vectorize(lambda l: self.label_lookup[l] if l in self.label_lookup else -1)

    def forward(self, x, labelset=''):
        # Encode Text
        if self.precomputed_text:  # We load the precomputed feats if flag is true
            if labelset == '':
                label_indexes = np.arange(self.text.shape[0]) # we take all
            else:
                label_indexes = self.label_index_mapper(labelset)  # we pick only the given ones

            text_features = self.precomputed_feats[label_indexes]
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        else:  # Or we do an inference pass every step to calculate
            if labelset == '':
                text = self.text
    ...
    def forward_image(self, x):
        ...
        out = self.scratch.output_conv(out).permute(0, 2, 3, 1).contiguous()

        return out, path_1.permute(0, 2, 3, 1).contiguous(), path_2.permute(0, 2, 3, 1).contiguous(), path_3.permute(0, 2, 3, 1).contiguous()
