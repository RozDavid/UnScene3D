...
def do_training(hparams, model_constructor):
    ...
    tb_logger = TensorBoardLogger("checkpoints", default_hp_metric=False, version=hparams.version)
...
def do_testing(hparams, model_constructor):
    # instantiate model
    model = model_constructor.load_from_checkpoint(hparams.weights, **vars(hparams))
    # set all sorts of training parameters
    hparams.gpus = -1
    hparams.accelerator = "ddp"
    hparams.benchmark = True
    ...
    if hparams.weights is None:
        ...
        hparams.resume_from_checkpoint = hparams.weights

    tb_logger = TensorBoardLogger("checkpoints", default_hp_metric=False)

    wblogger = pl.loggers.WandbLogger(
        save_dir="tests",
        project=hparams.project_name,
        name=hparams.exp_name,
    )

    hparams.logger = [wblogger, tb_logger]

    trainer = pl.Trainer.from_argparse_args(hparams)
    trainer.test(model, ckpt_path=hparams.weights)
...
def get_default_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If loading pretrained weights",
    )
    ...
    parser.add_argument(
        "--project_name", type=str, default="LSeg", help="project name for logging"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.4,
        help="The scale of the image size - 1. ratio is 480, 0.5 is 240 so on...",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If save output visualizations",
    )

    parser.add_argument(
        "--visualize_path", type=str, default="./visualize/", help="Output directory name"
    )
...
def make_checkpoint_callbacks(exp_name, version, base_path="checkpoints", frequency=1):
    ...
    val_iou_callback = pl.callbacks.ModelCheckpoint(
        monitor="validation/mIoU",
        dirpath=f"{base_path}/{exp_name}/version_{version}/checkpoints/",
        filename="result-{epoch}-{validation/mIoU:.2f}",
        mode="max",
        save_top_k=1,
        verbose=True,
    )

    val_acc_callback = pl.callbacks.ModelCheckpoint(
        monitor="validation/accuracy",
        dirpath=f"{base_path}/{exp_name}/version_{version}/checkpoints/",
        filename="result-{epoch}-{validation/accuracy:.2f}",
        mode="max",
        save_top_k=1,
        verbose=True,
    )

    return [base_callback, val_iou_callback, val_acc_callback]
...
def set_resume_parameters(hparams):
    ...
    if version is not None:
        ...
        hparams.weights = latest
        hparams.version = version

        wandb_file = f"checkpoints/{hparams.exp_name}/version_{version}/wandb_id"
...
def get_wandb_logger(hparams):
    ...
    if os.path.exists(id_file) and not hparams.no_resume:
